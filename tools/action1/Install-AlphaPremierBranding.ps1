...
$DownloadUrl = 'https://github.com/Deign86/Alpha_Branding/releases/latest/download/AlphaBranding-1.6.3.zip'
...
try {
    ...
    if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'The script must run as Administrator or LocalSystem.' }
    ...
    foreach ($folder in @('publish', 'app-files', 'Alpha.Branding')) { $candidate = Join-Path $ExtractRoot $folder; if (Test-Path $candidate -PathType Container) { $PayloadRoot = $candidate; break } }
    $AppExe = Get-ChildItem -Path $PayloadRoot -Filter '*.exe' -File -Recurse | Where-Object { $_.Name -notmatch 'Setup|Installer|Uninstall|Bootstrapper' } | Select-Object -First 1
